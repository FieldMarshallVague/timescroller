...
import { Connection, Endpoint } from './js-plumb';

export type Selector = string;
export type UUID = string;
export type ElementId = string;
export type ElementRef = ElementId | Element;
export type ElementGroupRef =
  | ElementId
  | Element
  | Array<ElementId>
  | Array<Element>;

export interface LeftTopLocation {
  left: number;
  top: number;
}
...
export interface PlumbDefaults {
...
export class PlumbDefaultsFactory {
  getDefaults(): PlumbDefaults {
    return {
      Anchor: 'Bottom',
      Anchors: [null, null],
